  ...
  const _connect = useCallback(
    ({
      ...
      socket.on('update_message', (message: IStep) => {
        alert(`update_message,message.id:${message.id}`)
        setMessages((oldMessages) =>{
          const result = updateMessageById(oldMessages, message.id, message);
          if(JSON.stringify(result).includes("element.name")) alert("element.name")
          return result;
        });
      });
